function numerical_test_MM_robustness_branch2()
...
%% three enzyme branch+linear system: limited upstream metabolite
...
dScdt=k0             - k*E1c*Sc/(1+Sc)       - k*E3c*Sc/(1+Sc) -      beta*Sc - eta*(Sc-So)*gamma;
dSodt=               - theta*k*E1o*So/(1+So)            -      beta*So - eta*(So-Sc);

dIcdt=k*E1c*Sc/(1+Sc)       - k*E2c*Ic/(1+Ic)                  - beta*Ic - eta*(Ic-Io)*gamma;
...
dBcdt=                 k*E3c*Sc/(1+Sc)                  -      Bc - eta*(Bc-Bo)*gamma;
dBodt=                                           -      Bo - eta*(Bo-Bc);
